...
class NsgPhoneLoginPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body:
          NsgPhoneLoginWidget(this, loginProvider, widgetParams: widgetParams),
    );
  }
  ...
  Widget getRememberMeCheckbox(BuildContext context) {
    bool initialValue = loginProvider.provider.saveToken;
    var checkbox = NsgCheckBox(
      ...
      onPressed: (currentValue) {
        loginProvider.provider.saveToken = currentValue;
      },
      value: initialValue,
    );
    return checkbox;
...
class NsgPhoneLoginWidget extends StatefulWidget {
  @override
  NsgPhoneLoginWidgetState createState() => NsgPhoneLoginWidgetState();

  final NsgPhoneLoginPage loginPage;
  final NsgPhoneLoginParams? widgetParams;
  //final NsgDataProvider provider;
  final NsgLoginProvider loginProvider;
  const NsgPhoneLoginWidget(this.loginPage, this.loginProvider,
      {super.key, this.widgetParams});
}

class NsgPhoneLoginWidgetState extends State<NsgPhoneLoginWidget> {
  Image? captureImage;
  String phoneNumber = '';
  String email = '';
  String captchaCode = '';
  bool isCaptchaLoading = false;
  int currentStage = NsgPhoneLoginWidgetState.stagePreLogin;
  ...
  void initState() {
    super.initState();
    widget.loginPage.callback.sendDataPressed = (context) => doSmsRequest(
        context: context, loginType: loginType, password: password);
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  if (kIsWeb || (!Platform.isAndroid && !Platform.isIOS))
                    widget.loginPage.getRememberMeCheckbox(context),
                  ...
                  if (widget.widgetParams!.usePasswordLogin)
                    widget.loginPage.getButtons(context),
                  if (!widget.widgetParams!.usePasswordLogin)
                    NsgButton(
                      margin: EdgeInsets.zero,
                      onPressed: () {
...
/* -------------------------------------------------------------- Если введён пароль -------------------------------------------------------------- */
  ...
  Widget _getRegistrationWidget() {
    return getRegistrationWidget!(widget.loginProvider);
  }

  Widget _getVerificationWidget() {
    return getVerificationWidget!(widget.loginProvider);
  }
