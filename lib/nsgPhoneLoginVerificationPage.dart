...
import 'authorization_params.dart';
...
class NsgPhoneLoginVerificationPage extends StatelessWidget {
  final NsgLoginProvider loginProvider;
  final NsgPhoneLoginParams? widgetParams;

  NsgPhoneLoginVerificationPage(this.loginProvider,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: NsgPhoneLoginVerificationWidget(this, loginProvider,
...
class NsgPhoneLoginVerificationWidget extends StatefulWidget {
  ...
  const NsgPhoneLoginVerificationWidget(
      this.verificationPage, this.loginProvider,
      {super.key, this.widgetParams});
  @override
...
class _NsgPhoneLoginVerificationState
  ...
  @override
  void initState() {
    widget.verificationPage.callback.sendDataPressed = (context) =>
        doSmsRequest(
            context: context,
            loginType: widget.widgetParams!.usePhoneLogin
                ? NsgLoginType.phone
                : NsgLoginType.email);
    super.initState();
    startTimer();
  ...
  void startTimer() {
    updateTimer ??= updateTimer = Timer.periodic(
        const Duration(seconds: 1), (Timer t) => updateTimerEvent(t));
  }
  ...
  Widget _getBody(BuildContext context) {
    return Stack(
      children: <Widget>[
        Positioned.fill(
          child: widget.verificationPage.getBackground(context),
        ),
        Align(
          alignment: Alignment.center,
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                Container(
                  child: widget.verificationPage.getLogo(context),
                ),
                Container(
    ...
    return Form(
      ...
      child: Container(
        decoration: BoxDecoration(
            color: nsgtheme.colorMainDarker,
                ...
                children: <Widget>[
                  ...
                  Text(
                    widget.widgetParams!.interpolate(
                        ...
                        params: {'phone': widget.loginProvider.phoneNumber}),
                  ...
                  TextFormField(
                    ...
                    decoration: InputDecoration(
                      ...
                      hintStyle:
                          TextStyle(color: nsgtheme.colorText.withOpacity(0.3)),
                      enabledBorder: OutlineInputBorder(
                        borderSide: BorderSide(
                            color: nsgtheme.colorText.withOpacity(0.5),
                            width: 1.0),
                      ),
                      focusedBorder: OutlineInputBorder(
                        borderSide:
                            BorderSide(color: nsgtheme.colorText, width: 1.0),
                      ),
                    ),
                    onChanged: (text) {
                      securityCode = text;
                  ...
                  const SizedBox(height: 15.0),
                  widget.verificationPage.getButtons(context),
                        ...
                        children: [
                          TextFormField(
                            ...
                            decoration: InputDecoration(
                              ...
                              errorStyle: const TextStyle(fontSize: 12),
                              hintText:
                                  widget.widgetParams!.textEnterNewPassword,
                              hintStyle: TextStyle(
                          ...
                          Padding(
                            ...
                            child: TextFormField(
                              ...
                              decoration: InputDecoration(
                                ...
                                errorStyle: const TextStyle(fontSize: 12),
                                hintText:
                                    widget.widgetParams!.textEnterPasswordAgain,
                                enabledBorder: OutlineInputBorder(
                                  borderSide: BorderSide(
                                      color:
                              ...
                              child: NsgButton(
                                ...
                                onPressed: () {
                                  if (newPassword != newPassword2) {
                                    ...
                                    nsgSnackbar(context,
                                        text:
                                            'Введите новый пароль в оба текстовых поля');
                                  } else {
                                    widget.loginProvider
                                        ...
                                        .then((result) =>
                                            checkLoginResult(context, result));
                                  }
                                },
                  ...
                  if (!widget.widgetParams!.usePasswordLogin)
                    Padding(
                      ...
                      child: NsgButton(
                        ...
                        onPressed: () {
                          if (newPassword != newPassword2) {
                            nsgSnackbar(context, text: 'Пароли не совпадают');
                          } else {
                            widget.loginProvider
                                .phoneLogin(
                                    phoneNumber:
                                        widget.loginProvider.phoneNumber,
                                    securityCode: securityCode,
                                    register: true,
                                    newPassword: newPassword)
                                .then((result) =>
...
/* -------------------------------------------------------------- Если введён пароль -------------------------------------------------------------- */
    ...
    widget.loginProvider
        .phoneLoginRequestSMS(
            phoneNumber: widget.widgetParams!.usePhoneLogin
                ? widget.widgetParams!.phoneNumber
                : widget.widgetParams!.email,
  ...
  void checkRequestSMSanswer(
    ...
    if (answerCode.errorCode == 40300) {
      setState(() {});
      nsgSnackbar(context, text: answerCode.errorMessage);
      return;
    }
    if (answerCode.errorCode == 0) {
      ...
      nsgSnackbar(context,
          text:
              '${answerCode.secondsRemaining} ${answerCode.secondsBeforeRepeat}');
  ...
  Widget _getLoginWidget() {
    return getLoginWidget!(widget.loginProvider);
  }
  ...
  void checkLoginResult(BuildContext context, NsgLoginResponse answer) {
    ...
    if (answerCode != 0) {
      ...
      NsgNavigator.instance
          .offAndToPage(context, widget.widgetParams!.mainPage);
    }
  }
  ...
  void updateTimerEvent(Timer t) {
    if (widget.loginProvider.provider.smsRequestedTime == null) {
      stopTimer();
    }
    setState(() {
      secondsRepeateLeft = 120 -
          DateTime.now()
              .difference(widget.loginProvider.provider.smsRequestedTime!)
              .inSeconds;
